...
const { adminDashboard, loading, fetchAdminDashboard } = useDashboard()
...
const animateTotals = () => {
  if (adminDashboard.value) {
    countUp(adminDashboard.value.totalWallets, animatedWallets)
    countUp(adminDashboard.value.totalContracts, animatedContracts)
    countUp(adminDashboard.value.totalDocuments, animatedDocuments)
  }
}

onMounted(async () => {
  await fetchAdminDashboard()
  console.log(adminDashboard.value)
  animateTotals()
})
</script>
...
<template>
  ...
  <!-- Recent Contracts -->
  <section v-if="adminDashboard?.recentContracts?.length" class="mt-8">
    ...
    <div class="overflow-x-auto bg-white dark:bg-gray-900 rounded-xl shadow ring-1 ring-gray-200 dark:ring-gray-700">
      <table class="w-full table-auto text-left">
        ...
        <tbody>
          <tr
            v-for="c in adminDashboard.recentContracts"
            :key="c.address"
  ...
  <!-- Recent Documents -->
  <section v-if="adminDashboard?.recentDocuments?.length" class="mt-8">
    ...
    <div class="overflow-x-auto bg-white dark:bg-gray-900 rounded-xl shadow ring-1 ring-gray-200 dark:ring-gray-700">
      <table class="w-full table-auto text-left">
        ...
        <tbody>
          <tr
            v-for="d in adminDashboard.recentDocuments"
            :key="d.id"
            class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
          >
