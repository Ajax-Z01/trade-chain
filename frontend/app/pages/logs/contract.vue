...
<template>
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    <!-- Empty -->
    <div v-else-if="logs.length === 0" class="text-gray-500 text-center py-10">
      No contracts found.
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        <!-- Header -->
        <button
          class="w-full px-4 py-3 flex justify-between items-center bg-gray-50 hover:bg-gray-100 focus:outline-none transition"
          @click="toggleContract(contract)"
        >
          <span class="font-medium truncate">Contract: {{ contract }}</span>
          <ChevronRight
            :class="getContractState(contract).isOpen ? 'rotate-90 text-indigo-600' : 'text-gray-400'"
              ...
              <table class="min-w-full border-collapse border border-gray-200">
                ...
                <tbody>
                  <tr
                    ...
                    :key="i"
                    class="bg-white even:bg-gray-50 hover:bg-indigo-50 transition-colors duration-150"
                    :class="isRecent(log.timestamp) ? 'border-l-4 border-indigo-400' : ''"
                  >
                    <td class="border px-3 py-2 text-sm text-gray-700">{{ formatDate(log.timestamp) }}</td>
