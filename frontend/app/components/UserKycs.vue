...
const fetchUserKycs = async () => {
  ...
  try {
    ...
    console.error(err)
    error.value = err.message || 'Failed to fetch your NFTs'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div v-if="props.approvedMintersKYC && props.account && !isAdmin" class="p-4 space-y-4">
    <h3 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Your KYCs</h3>

    <p v-if="loading" class="text-gray-500 dark:text-gray-400">Loading your KYCs...</p>
    ...
    <div v-if="userKycs.length" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div v-for="nft in userKycs" :key="nft.tokenId" class="p-4 rounded-xl bg-white dark:bg-gray-900 shadow border space-y-2">
        <div class="flex justify-between items-start">
          <div>
            <p><strong>Token ID:</strong> {{ nft.tokenId }}</p>
            <p><strong>Status:</strong>
              <span :class="statusClass(nft.status)" class="px-2 py-1 rounded-full text-sm font-medium">{{ nft.status }}</span>
            </p>
          </div>
          <img v-if="nft.metadataUrl" :src="nft.metadataUrl" class="w-24 h-24 object-cover rounded border dark:border-gray-600" />
        </div>
